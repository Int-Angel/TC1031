/*
	Actividad 2.3
		...
		y una fecha de fin para hacer una busqueda en ese rango de fechas, posteriormente se almacenan en un diccionario los registros,
		tomando como parámetros clave la fecha y la ip, mientras que en el valor se almacena la cantidad de accesos.
		Posteriormente se ordenan los resultados en orden descendente por fecha, y por la cantidad de accesos.
		Finalmente se imprimen los registros y se almacenan en resultados.txt
...
public:
	...
	/// </summary>
	/// <returns>retorna la fecha en string</returns>
...
vector<Log> readfile();
void sort(map<tuple<string, string>, int> &M, ofstream &file);
...
void imprimirRegistros(vector<Log> logs, int i1, int i2)
{
	...
	int currentDate = logs[i2].getSimpleDateValue();
	map<tuple<string, string>, int> mymap;
	...
	{
		//llenamos map
		if (currentDate != logs[i].getSimpleDateValue())
		{
			sort(mymap, file);
			currentDate = logs[i].getSimpleDateValue();
			mymap.clear();
		}

		it = mymap.find(tuple<string, string>(logs[i].getDateString(), logs[i].getIp()));
		if (it != mymap.end())
			mymap[tuple<string, string>(logs[i].getDateString(), logs[i].getIp())]++;
		else
		{
			mymap[tuple<string, string>(logs[i].getDateString(), logs[i].getIp())] = 1;
		}
		count++;
		//termina los datos de un dia y pasa a la siguiente
	}
	sort(mymap, file);
...
// Funcion de comparación para ordenar el set mediante el incremento
// del orden del segundo valor del par.
struct comp
{
	template <typename T>
...
// Función sort.
// Se encarga de ordenar un mapa en base a una tupla.
void sort(map<tuple<string, string>, int> &M, ofstream &file)
{
	// Declare set of pairs and insert
	// pairs according to the comparator
	// function comp()
	set<pair<tuple<string, string>, int>, comp> S(M.begin(),
												  M.end());

	// Print the sorted value
	set<pair<tuple<string, string>, int>>::iterator it;
	for (it = S.begin(); it != S.end(); it++)
	{
		cout << get<0>(it->first) << " " << it->second << " " << get<1>(it->first) << endl;
		file << get<0>(it->first) << " " << it->second << " " << get<1>(it->first) << endl;
	}
}
